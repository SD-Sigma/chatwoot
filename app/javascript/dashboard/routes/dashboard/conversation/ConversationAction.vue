...
export default {
    ...
    computed: {
        ...
        teamsList() {
            ...
            let filteredTeams = this.availableTeams;

            // Si el equipo "bot automatizado" (id: 18) está seleccionado, mostrarlo
            // Si no está seleccionado, filtrarlo de la lista
            const isBotTeamSelected = this.assignedTeam && this.assignedTeam.id === 18;
            
            if (!isBotTeamSelected) {
                filteredTeams = filteredTeams.filter(team => team.id !== 18);
            }

            // Agregar opción "Ninguno" si hay un equipo asignado
            if (this.hasAnAssignedTeam) {
                return [{ id: 0, name: this.$t('TEAMS_SETTINGS.LIST.NONE') }, ...filteredTeams];
            }
